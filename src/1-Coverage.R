...
dotchart(cov[[1]]$Depth/Coverage[1,]$depth2, 
    ...
    xlab = expression(paste("Segment depths relatively to the depth of ", italic('C. typhae'))))
for (i in 2:length(cov)){
    points(cov[[i]]$Depth, 1:nrow(cov[[1]]), col = colors[i], pch = 19, cex = 0.8)
}
legend(90, y=17, legend = names, col = colors, fill = colors, pch=19, cex = 0.8)
dev.off()
